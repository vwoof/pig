...
class ArticleController extends HomebaseController {
    ...
    public function index() {
    	...
    	if ($_SESSION['user'] != null)
    	{
        	...
        	// 判断是否有支付这个
        	$order_db = M('orders');
        	$order_data = $order_db->where("params=$article_id and is_payed=1 and user_id=" . $_SESSION['user']['id'])->find();
        	if ($order_data != null)
        	    $is_payed = 1;
    	}
    	...
    	$tickets_arr = $article['tickets_arr'];
    	$tickets_arrs = explode(',', trim($tickets_arr));
    	$random_start_index = rand(393000, 393760);
    	$wx_users_db = M('wx_users');
    	$tickets_users = $wx_users_db->where()->field('user_nicename, avatar')->limit($random_start_index, count($tickets_arrs) + 3)->select();
    	$tickets_data_arr = array();
    	for ($i=0; $i<count($tickets_arrs); $i++)
    	{
    	    $data = array();
    	    $data['type'] = rand(0, 1);
    	    $data['name'] = $tickets_users[$i]['user_nicename'];
    	    $data['avatar'] = $tickets_users[$i]['avatar'];
    	    $tickets_data_arr[$tickets_arrs[$i]] = $data;
    	}
    	$article['ticket_users'] = json_encode($tickets_data_arr);
