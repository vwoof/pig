    	...
    	if (empty($pagesize)) {
    	    ...
    	    if (session('city') != null)
    	    {
    	        for ($j=0; $j<count($posts); $j++)
    	        {
    	            ...
    	            $posts[$i]['post_address'] = str_replace('{city}', session('city'), $posts[$i]['post_address']);
    	            
    	            if ($posts[$i]['status'] == 0)
    	    		{
    	    		    if ($count < $max_count)
    	    		    {
    	    		        $has_coins = $posts[$i]['need_coins'];
    	    		        $posts[$i]['count'] = rand(100, 9999);
    	    		        $posts[$i]['status'] = 1;
    	    		        $posts[$i]['has_coins'] = $has_coins;
    	    		        $posts[$i]['sort_val'] = (2 - $posts[$i]['status']) * 10000 + $has_coins;
    	    		        $count++;
    	    		    }
    	    		    else
    	    		    {
    	    		        ...
    	    		        {
            	    			$has_coins = rand(1, $posts[$i]['need_coins'] - 2);
            	    			$posts[$i]['count'] = rand(100, 9999);
            	    			$posts[$i]['has_coins'] = $has_coins;
            	    			$posts[$i]['sort_val'] = (2 - $posts[$i]['status']) * 10000 + $has_coins;
    	    		        }
    	    ...
    	    {	        
    	        for ($j=0; $j<count($posts); $j++)
    	        {
    	    		...
    	    		$posts[$i]['post_address'] = str_replace('{city}', '深圳市', $posts[$i]['post_address']);
    	    ...
    	    quickSort($posts, 0, count($posts) - 1, 'sort_val');
