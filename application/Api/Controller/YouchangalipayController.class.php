...
private $wx_pay_db = null;
	...
	function _initialize(){
        ...
        public function notify2312() {
            ...
            \Log::DEBUG('YouchangalipayController:' . $xml);
        ...
        {
            \Log::DEBUG('YouchangalipayController:' . $order_sn);
            ...
            if ($order['status'] == 0)
            {
                ...
                if (intval($order['price']) != $total_fee)
                {
                    $data = array(
                        'id' => $order['id'],
                        'status' => 2,
                    		'from_source' => 'UCHANG_ALI:' .C('YOUCHANG_ALI_MCHID'),
                        'memo' => '错误金额:' . $total_fee,
